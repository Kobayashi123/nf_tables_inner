...
static inline bool mt_seg6_encap(const struct sk_buff *skb, int innoff,
				 const struct ip6t_ip6 *ip6info)
{
	struct ipv6hdr _inner_hdr;
	const struct ipv6hdr *inner_hdr;

	inner_hdr = skb_header_pointer(skb, innoff, sizeof(_inner_hdr),
				       &_inner_hdr);
...
static inline bool mt_seg6_insert(const struct sk_buff *skb, int srhoff,
				  const struct ip6t_ip6 *ip6info)
{
	int hdrlen;
	...
	if (NF_INVF(ip6info, IP6T_INV_SRCIP,
		    ipv6_masked_addr_cmp(&ipv6_hdr(skb)->saddr, &ip6info->smsk,
					 &ip6info->src)) ||
	    NF_INVF(ip6info, IP6T_INV_DSTIP,
		    ipv6_masked_addr_cmp(daddr, &ip6info->dmsk, &ip6info->dst)))
		return false;
	return true;
}

static inline bool inner_match(const struct sk_buff *skb, int srhoff,
			       int innoff, int encap,
			       const struct ip6t_ip6 *ip6info)
{
	if (encap)
...
static inline bool sr6_pre_processor(const struct sk_buff *skb, int *innoff,
				     int *srhoff, int *encap)
{
	if (ipv6_find_hdr(skb, innoff, IPPROTO_IPV6, NULL, NULL) ==
	    IPPROTO_IPV6) {
		*encap = 1;
		return true;
	}
	if (ipv6_find_hdr(skb, srhoff, IPPROTO_ROUTING, NULL, NULL) ==
	    IPPROTO_ROUTING)
		return true;
	return false;
}
...
ip6_packet_match(const struct sk_buff *skb,
		 ...
		 u16 *fragoff, bool *hotdrop, struct net *net)
{
	unsigned long ret;
